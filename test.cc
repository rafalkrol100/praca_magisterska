...
int main(int argc, char *argv[])
{
    ...
    NodeContainer enbNodesOutter;
    enbNodesOutter.Create(NUMBER_OF_OUTTER_STATIONS);
    NodeContainer ueNodes;
    ueNodes.Create(NUMBER_OF_UES);

    // lists of devices positions
    Ptr<ListPositionAllocator> enbPositionAlloc = CreateObject<ListPositionAllocator>();
    Ptr<ListPositionAllocator> enbOutterPositionAlloc = CreateObject<ListPositionAllocator>();
    ...
    for (int i = NUMBER_OF_STATIONS; i < NUMBER_OF_STATIONS + NUMBER_OF_OUTTER_STATIONS; i++)
    {
        if (isCompEnabled)
        {

            enbOutterPositionAlloc->Add(stationsPositions[i]);
        }
        else
        {
            enbOutterPositionAlloc->Add(stationsPositions[i]);
    ...
    MobilityHelper enbOutterMobility;
    enbMobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    enbOutterMobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    enbMobility.SetPositionAllocator(enbPositionAlloc);
    enbOutterMobility.SetPositionAllocator(enbOutterPositionAlloc);
    enbMobility.Install(enbNodes);
    enbOutterMobility.Install(enbNodesOutter);
    ...
    for (int i = 0; i < NUMBER_OF_STATIONS + NUMBER_OF_OUTTER_STATIONS; i++)
    {
    ...
    for (int i = 0; i < NUMBER_OF_STATIONS; i++)
    {
        ...
        {
            NS_LOG_UNCOND("    Antenna Model: ns3::IsotropicAntennaModel");
    ...
    for (int i = 0; i < NUMBER_OF_OUTTER_STATIONS; i++)
    {
        NS_LOG_UNCOND("Enb" + std::to_string(i + 7) + ": ");

        NS_LOG_UNCOND("    Antenna Model: ns3::IsotropicAntennaModel");
        enbDevs.Add(lteHelper->InstallEnbDevice(enbNodesOutter.Get(i)));
        Ptr<Node> enb = enbNodesOutter.Get(i);
        Ptr<NetDevice> enbLteDev = enb->GetDevice(0);
        Ptr<LteEnbNetDevice> enbLteDevice = enbLteDev->GetObject<LteEnbNetDevice>();
        uint16_t enbCellId = enbLteDevice->GetCellId();
        NS_LOG_UNCOND("    Cell Id: " + std::to_string(enbCellId));
    ...
    remHelper->SetAttribute("XMin", DoubleValue(-480.0));
    remHelper->SetAttribute("XMax", DoubleValue(480.0));
    remHelper->SetAttribute("YMin", DoubleValue(-480.0));
    remHelper->SetAttribute("YMax", DoubleValue(480.0));
